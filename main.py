...
class Interface:
    def __init__(self, root):
        ...
        self.main_window.bind('<Up>', lambda *ignore : self.button_event('up'))
        self.main_window.bind('<Down>', lambda *ignore : self.button_event('down'))
        self.main_window.bind('<Left>', lambda *ignore : self.button_event('left'))
        self.main_window.bind('<Right>', lambda *ignore : self.button_event('right'))

        self.canvas = Canvas(self.main_window, bg='#776e65', bd=0, highlightthickness=0, relief='ridge', width=450, height=450)
        self.canvas.place(x=0, y=0)

        self.blocks = [[] for _ in range(4)]
        self.colors = {'0' : ['#fafafa', '#776e65'], '2' : ['#eee4da', '#776e65'], '4' : ['#ede0c8', '#776e65'],
    ...
    def create_start_blocks(self):
        for y in range(4):
            for x in range(4):
                self.blocks[y].append(Canvas_block(self.canvas, 110*x + 10, 110*y + 10))
        self.start_spaun()
    ...
    def return_indexes(self, button, iteration, reverse=False):
        ...
        reverse_false = {'left' : [([i, 3 - iteration], [i, 2 - iteration]) for i in range(4)],
    ...
    def lose_condition(self):
        for i in range(3):
            vertical, horizontal = self.return_indexes('up', i, reverse=True), self.return_indexes('left', i, reverse=True)
            for j in range(4):
                first_v, second_v = self.blocks[vertical[j][0][0]][vertical[j][0][1]], self.blocks[vertical[j][1][0]][vertical[j][1][1]]
                first_h, second_h = self.blocks[horizontal[j][0][0]][horizontal[j][0][1]], self.blocks[horizontal[j][1][0]][horizontal[j][1][1]]
...
class Canvas_block:
    def __init__(self, canvas, x, y):
        ...
        self.y = y
        self.canvas = canvas
        self.interface_object = canvas.create_rectangle(self.x, self.y, self.x + 100, self.y + 100, fill='#fafafa', outline='')
